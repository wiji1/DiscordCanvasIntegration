...
public:
    ...
    static  void init(const char* html, const std::string& file_name);

    static std::string replace_unicode_escapes(const std::string &input);

    static void post_announcement_embed(long channel_id, const std::string &html, const std::string &title, const std::string &url, const std::string &author);
};
